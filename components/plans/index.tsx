    ...
    return (
        <Flex
            ...
            css={{
                ...
                p: '$10',
                position: 'relative',
                bg: recommended ? (isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.03)') : 'transparent',
        ...
        >
            ...
            <Box css={{ mb: '$8' }}>
                <Flex justify={'between'} align={'center'}>
                    <Text h4 css={{ color: recommended ? '$text' : '$accents6', textTransform: 'uppercase', letterSpacing: '1px', fontSize: '$xs', m: 0 }}>
                        {title}
                    </Text>
                    {recommended && (
                        <Text span css={{ fontSize: '10px', fontWeight: 'bold', bg: '$primary', color: 'white', px: '6px', py: '2px', borderRadius: '4px' }}>
                            POPULAR
                        </Text>
                    )}
                </Flex>
                <Text h2 css={{ fontSize: '2.5rem', lh: 1, mt: '$4', mb: '$2', color: '$text' }}>{price}</Text>
                ...
                <Button
                    ...
                    css={{
                        width: '100%',
                        bg: recommended ? '$text' : (isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)'),
                        color: recommended ? '$background' : '$text',
                        fontWeight: '700',
                        border: isDark ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(0,0,0,0.1)',
                        '&:hover': {
                            bg: recommended ? '$accents8' : (isDark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.1)')
                        }
                    }}
                >
            ...
            <Box as={'ul'} css={{ m: 0, p: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '$3' }}>
                {features.map((feature, i) => (
                    <Flex key={i} css={{ gap: '$3' }} align={'center'}>
                        <CheckIcon fill={recommended ? "#0072F5" : (isDark ? "#888" : "#666")} width={16} height={16} />
        ...
        <Box css={{ py: '$24', px: '$6', id: 'pricing', maxWidth: '1200px', margin: '0 auto' }}>
            ...
            <Box
                css={{
                    ...
                    bg: isDark ? 'rgba(5,5,5,0.6)' : 'rgba(255,255,255,0.8)'
                }}
            >
                <PlanColumn
                    title="Growth Launch"
                ...
                />
                {/* Divider Desktop */}
                <Box css={{ width: '1px', bg: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)', display: 'none', '@md': { display: 'block' } }} />
                {/* Divider Mobile */}
                <Box css={{ height: '1px', bg: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)', display: 'block', '@md': { display: 'none' } }} />
